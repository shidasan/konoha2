...
CHKSTACK   0                 n:sfpidx
LDMTD      0                 thisidx:sfpidx loadmtd:f cache:hcache mtd:Method
SCALL      _DEF|_JIT         uline:u thisidx:ro espshift:ro mtd:Method tyo:co
VCALL      _DEF|_JIT         uline:u thisidx:ro espshift:ro mtd:Method tyo:co
VCALL_     _DEF|_JIT         uline:u thisidx:ro espshift:ro mtd:Method tyo:co
#FASTCALL0  _DEF|_JIT         uline:u thisidx:ro rix:i espshift:sfpidx fcall:f
...
{''')
	for kc in KCODE_LIST:
		f.write('''
	assert(sizeof(%s) <= sizeof(kopl_t));''' % (kc.ctype))
	f.write('''
}
