...
#include <konoha2.h>   // added to integrate konoha
		...
		void* thunk,int (*cmp)(void* ,const void*,const void*))
{
	...
	if (total_elems > MAX_THRESH)
	{
		...
		while (STACK_NOT_EMPTY)
		{
			...
			if ((*cmp) (thunk, (void *) mid, (void *) lo) < 0)
				SWAP (mid, lo, size);
			if ((*cmp) (thunk, (void *) hi, (void *) mid) < 0)
				SWAP (mid, hi, size);
			else
				goto jump_over;
			if ((*cmp) (thunk, (void *) mid, (void *) lo) < 0)
			...
			do
			{
				...
				while ((*cmp) (thunk, (void *) mid, (void *) right_ptr) < 0)
					right_ptr -= size;
...
#define min(x, y) ((x) < (y) ? (x) : (y))
#endif
	{
		...
		for (run_ptr = tmp_ptr + size; run_ptr <= thresh; run_ptr += size)
			if ((*cmp) (thunk, (void *) run_ptr, (void *) tmp_ptr) < 0)
				tmp_ptr = run_ptr;
		...
		{
			tmp_ptr = run_ptr - size;
			while ((*cmp) (thunk, (void *) run_ptr, (void *) tmp_ptr) < 0)
				tmp_ptr -= size;
